...
KEY_DIR=/root/homelab-borgbackup-admin-recovery
...
if [ -f "$KEY_PATH" ]; then
    ...
    ssh-keygen -t ed25519 -N "" -f "$KEY_PATH" -C "homelab-borgbackup-admin-recovery" >/dev/null
    chmod 600 "$KEY_PATH"
...
echo
echo "    homelab-borgbackup list-archives --host <identifier> --server $(hostname) \\"
